...
/**
  ...
  */
	 ...
	 * Tutorial 8-1: Usage of the UART/USART module in blocking operation mode,
	 * the C code
	 ...
	 should use an application such as RealTerm
	 ***************************************************************************/
...
int main(void)
{
  ...
  while (1)
  {
	 // Send message from board
	 HAL_UART_Transmit(&huart1, mymessage, sizeof(mymessage), HAL_MAX_DELAY);
	 sprintf(cnt_buf, "%u\r\n", counter++);
     // send counter value     HAL_UART_Transmit(&huart1, (uint8_t*) cnt_buf, sizeof(cnt_buf), HAL_MAX_DELAY);
     // Toggle led
