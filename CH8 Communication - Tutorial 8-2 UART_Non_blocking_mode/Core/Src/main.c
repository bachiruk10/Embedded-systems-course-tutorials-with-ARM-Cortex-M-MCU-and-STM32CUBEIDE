...
/**
  ...
  */
	 ...
	 * Tutorial 8-2: Usage of the UART/USART module in non-blocking operation mode,
	 * the C code

	 In this tutorial, we configure stm32 to receive characters through UART from PC
	 Build the code , so when when the character ’g’ is received,
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  // Enable UART with interrupt so, the CPU waits to receive something from UART
  // When this happens, when data is received, a callback function is executed
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	// __NOP() No operation instruction: consumes clock cycles but does not execute anything.
	// It is mainly used for timing purposes to force memory alignments to prevent hazards, or
	// to occupy a branch delay slot.
	__NOP();
